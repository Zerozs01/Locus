...
const mdPath = path.resolve(__dirname, '../documents/deepreserach/Ecology_data/thailand_central_ecology_provinces_2026.json');
...
const seedFiles = [
  'portalSeed_central1.ts',
  'portalSeed_central2.ts',
  'portalSeed_central3.ts'
];
...
for (const file of seedFiles) {
  ...
  for (const [provName, ecoData] of Object.entries(converted)) {
    ...
    if (provName === "Phra Nakhon Si Ayutthaya") nameAlias = "Ayutthaya";

    const searchKeys = [provName, provName.toLowerCase().replace(/ /g, ''), nameAlias, nameAlias.toLowerCase().replace(/ /g, '')];
    
    let subContent = null;
    let provChunkRegex = null;

    for (const key of searchKeys) {
      // Find the start of the province
      const idx = content.indexOf(`"${key}": {`);
      if (idx !== -1) {
        // Find the END of the province by locating \n  },\n  "
        // or \n};
    ...
    // Match exactly from "ecoIds" ... to the next major key like "knowledgeTips", "metadata", or "supplyPoints"
    // This will safely wipe out any corrupted syntax!
    ...
    if (subContent !== newSubContent) {
      ...
      changed = true;
      console.log(`  ✅ Fixed ${provName} in ${file}`);
    } else {
