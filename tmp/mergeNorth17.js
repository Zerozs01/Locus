...
const tipsData = mdMatch ? JSON.parse(mdMatch[1]) : {};
...
files.forEach(f => {
  ...
  try {
    ...
    for (const pId of Object.keys(data)) {
        allSeededIds.push(pId); // Store actual keys used in seeds
    }
...
// id: "chiangmai", "bangkok" etc in initialData map to varying cases in seed files?
// Let's rely on mapping `name` from initialData
const expectedNamesMatch = [...initialDataContent.matchAll(/name:\s*"([A-Za-z\s]+)"/g)].map(m => m[1]);

const norm = (str) => str.toLowerCase().replace(/[\s-]/g, '');
const normalizedExpectedNames = expectedNamesMatch.map(norm).filter(n => !['ภาคเหนือ','ภาคอีสาน','ภาคกลาง','ภาคตะวันตก','ภาคตะวันออก','ภาคใต้'].includes(n) && n !== 'bangkokmetropolis'); // bangkok mismatch
normalizedExpectedNames.push('bangkok');
