...
try {
  let raw = fs.readFileSync('documents/deepreserach/part2_ภาคเหนือ.md', 'utf8');
  ...
  console.log(`Found ${keys.length} provinces in part 2: ${keys.join(', ')}`);

  const chunks = [];
  for (let i = 0; i < keys.length; i += 6) { // let's do chunks of 6
    chunks.push(keys.slice(i, i + 6));
  }
